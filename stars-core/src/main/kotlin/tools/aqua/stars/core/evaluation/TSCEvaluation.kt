...
class TSCEvaluation<
    ...
    val totalEvaluationTime = measureTime {
      ...
      val globalCache = PredicateCache<E, T, S, U, D>()
        if (tscListToEvaluate.isNotEmpty())
            segments.computeWhile { evaluateSegment(segment = it, tscList = tscListToEvaluate, globalCache = globalCache) }
      }
   ...
   */
  private fun evaluateSegment(segment: S, tscList: List<TSC<E, T, S, U, D>>, globalCache: PredicateCache<E, T, S, U, D>): Boolean {
